...
function* handleLoginAction(action: LoginAction) {
  const serverAddress: string = yield select(serverAddressSelector);
  try {
    ...
    if (response.type === "redirect") {
      const location = adjustOpenIDReturnTo(response.location);
      window.location.href = location;
    } else if (response.type === "jwt") {
      yield put(webapiAuthenticated(response.authorization));
    }
  } catch (e) {
